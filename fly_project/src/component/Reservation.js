      ...
      <h2>Every destinations possible {date}</h2>
      {
        isDestOn ? destFlight.map((flight) =>
          <div key={flight.flight_icao}>
            ...
            <button>Réserver ce vol</button>
          </div>
        ) :
          flightsInfos.length > 100 ? uniqueArray(flightsInfos).map((flight) => <button key={flight.flight_icao} onClick={showDest} value={flight.arr_iata}><p >Destination : {flight.arr_iata}</p></button>) :
            flightsInfos.map((flight) => <p key={flight.flight_icao}>Destination : {flight.arr_iata}</p>)
      }
    </div>
